...
async function loadJwks() {
  ...
  if (config.cloudflareAccessJwksFile) {
    const parsed = JSON.parse(await readFile(config.cloudflareAccessJwksFile, "utf8"));
    jwksCache = { loadedAt: Date.now(), keys: parsed.keys || [] };
    return jwksCache.keys;
  }
  if (!config.cloudflareAccessTeamDomain) {
    throw Object.assign(new Error("Thiếu CLOUDFLARE_ACCESS_TEAM_DOMAIN."), { statusCode: 500, code: "ADMIN_AUTH_NOT_CONFIGURED" });
  }
  const teamDomain = config.cloudflareAccessTeamDomain.replace(/^https?:\/\//, "").replace(/\/+$/, "");
  const response = await fetch(`https://${teamDomain}/cdn-cgi/access/certs`);
  const parsed = await response.json();
  if (!response.ok || !Array.isArray(parsed.keys)) {
    throw Object.assign(new Error("Không tải được Cloudflare Access JWKS."), { statusCode: 502, code: "JWKS_UNAVAILABLE" });
  }
  jwksCache = { loadedAt: Date.now(), keys: parsed.keys };
  return jwksCache.keys;
}
...
export async function loadAdmins({ force = false } = {}) {
  ...
  if (parsed.version !== 1 || !parsed.admins || typeof parsed.admins !== "object" || Array.isArray(parsed.admins)) {
    throw new Error("config/admins.json không hợp lệ.");
  }
...
function issuerExpected() {
  const domain = config.cloudflareAccessTeamDomain.replace(/^https?:\/\//, "").replace(/\/+$/, "");
  return `https://${domain}`;
}
